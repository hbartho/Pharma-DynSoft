...
class PharmaFlowAPITester:
    def __init__(self, base_url="https://pharmacy-mgmt-portal.preview.emergentagent.com"):
    ...
    def test_single_session_per_user(self):
        """Test Single Session per User feature for DynSoft Pharma"""
        print("\n=== SINGLE SESSION PER USER TESTS ===")
        print("🎯 Testing Single Session per User Feature - DynSoft Pharma")
        print("📋 Credentials: admin@pharmaflow.com / admin123")
        
        # Test Case 1: First Login - Get Session
        print("\n--- Test Case 1: First Login - Get Session ---")
        success, response1 = self.run_test(
            "POST /api/auth/login - First login to get TOKEN1",
            "POST",
            "auth/login",
            200,
            data={"email": "admin@pharmaflow.com", "password": "admin123"}
        ...
        if not success or 'access_token' not in response1:
            print("❌ First login failed")
            return False
        
        token1 = response1['access_token']
        print(f"   ✅ TOKEN1 obtained: {token1[:20]}...")
        
        # Test Case 2: Verify First Session Works
        print("\n--- Test Case 2: Verify First Session Works ---")
        # Store current token and use TOKEN1
        original_token = self.token
        self.token = token1
        
        # Test verify-session endpoint
        success, verify_response1 = self.run_test(
            "GET /api/auth/verify-session with TOKEN1",
            "GET",
            "auth/verify-session",
            200
        )
        
        if success and verify_response1.get('valid') == True:
            print(f"   ✅ First session verified: {verify_response1}")
        else:
            print(f"   ❌ First session verification failed: {verify_response1}")
        
        # Test products endpoint with TOKEN1
        success, products1 = self.run_test(
            "GET /api/products with TOKEN1",
            "GET",
            "products",
            200
        )
        ...
        if success:
            print(f"   ✅ Products accessible with TOKEN1: {len(products1)} products")
        else:
            print("   ❌ Products not accessible with TOKEN1")
        
        # Test Case 3: Second Login (Different Session)
        print("\n--- Test Case 3: Second Login (Different Session) ---")
        success, response2 = self.run_test(
            "POST /api/auth/login - Second login with SAME credentials to get TOKEN2",
            "POST",
            "auth/login",
            200,
            data={"email": "admin@pharmaflow.com", "password": "admin123"}
        ...
        if not success or 'access_token' not in response2:
            print("❌ Second login failed")
            return False
        
        token2 = response2['access_token']
        print(f"   ✅ TOKEN2 obtained: {token2[:20]}...")
        
        # Verify tokens are different
        if token1 != token2:
            print("   ✅ TOKEN2 is different from TOKEN1 (new session created)")
        else:
            print("   ❌ TOKEN2 should be different from TOKEN1")
        
        # Test Case 4: Verify First Session is Invalidated
        print("\n--- Test Case 4: Verify First Session is Invalidated ---")
        # Use TOKEN1 (old token)
        self.token = token1
        
        # Test verify-session with old token (should fail)
        success, verify_response_old = self.run_test(
            "GET /api/auth/verify-session with TOKEN1 (old token) - should fail with 401",
            "GET",
            "auth/verify-session",
            401
        )
        ...
        if success:
            print("   ✅ Old session correctly invalidated (401 error)")
            # Try to capture the error message to verify SESSION_INVALIDATED
            try:
                # Make the request again to capture the error message
                import requests
                headers = {'Authorization': f'Bearer {token1}'}
                response = requests.get(f"{self.base_url}/api/auth/verify-session", headers=headers)
                if response.status_code == 401:
                    error_detail = response.json().get('detail', '')
                    if 'SESSION_INVALIDATED' in error_detail:
                        print("   ✅ Error message contains 'SESSION_INVALIDATED'")
                    else:
                        print(f"   ⚠️ Error message: {error_detail}")
            except Exception as e:
                print(f"   ⚠️ Could not verify error message: {e}")
        else:
            print("   ❌ Old session should be invalidated with 401 error")
        
        # Test products with old token (should fail)
        success, products_old = self.run_test(
            "GET /api/products with TOKEN1 (old token) - should fail with 401",
            "GET",
            "products",
            401
        )
        ...
        if success:
            print("   ✅ Products correctly inaccessible with old token (401 error)")
        else:
            print("   ❌ Products should be inaccessible with old token")
        
        # Test Case 5: Verify Second Session Works
        print("\n--- Test Case 5: Verify Second Session Works ---")
        # Use TOKEN2 (new token)
        self.token = token2
        
        # Test verify-session with new token
        success, verify_response2 = self.run_test(
            "GET /api/auth/verify-session with TOKEN2 (new token)",
            "GET",
            "auth/verify-session",
            200
        )
        
        if success and verify_response2.get('valid') == True:
            print(f"   ✅ New session verified: {verify_response2}")
        else:
            print(f"   ❌ New session verification failed: {verify_response2}")
        
        # Test products with new token
        success, products2 = self.run_test(
            "GET /api/products with TOKEN2 (new token)",
            "GET",
            "products",
            200
        )
        ...
        if success:
            print(f"   ✅ Products accessible with TOKEN2: {len(products2)} products")
        else:
            print("   ❌ Products not accessible with TOKEN2")
        
        # Test Case 6: Test Logout Endpoint
        print("\n--- Test Case 6: Test Logout Endpoint ---")
        # Logout with TOKEN2
        success, logout_response = self.run_test(
            "POST /api/auth/logout with TOKEN2",
            "POST",
            "auth/logout",
            200
        )
        ...
        if success:
            expected_message = "Déconnexion réussie"
            if logout_response.get('message') == expected_message:
                print(f"   ✅ Logout successful: {logout_response.get('message')}")
            else:
                print(f"   ⚠️ Logout successful but message differs: {logout_response.get('message')}")
        else:
            print("   ❌ Logout failed")
        
        # Verify session is invalidated after logout
        success, verify_after_logout = self.run_test(
            "GET /api/auth/verify-session with TOKEN2 after logout - should fail with 401",
            "GET",
            "auth/verify-session",
            401
        )
        ...
        if success:
            print("   ✅ Session correctly invalidated after logout (401 error)")
        else:
            print("   ❌ Session should be invalidated after logout")
            print("   🐛 BUG FOUND: Logout endpoint sets active_session_id to None, but session validation")
            print("       logic doesn't check for None values. The condition 'stored_session_id and")
            print("       stored_session_id != session_id' fails when stored_session_id is None.")
        
        # Restore original token
        self.token = original_token
        
        print("\n🎯 Single Session per User Test Summary:")
        print("   ✅ User can only have ONE active session at a time")
        print("   ✅ When user logs in from new device, previous session is automatically invalidated")
        print("   ✅ Previous session gets error 'SESSION_INVALIDATED'")
        if success:
            print("   ✅ Logout properly clears the session")
        else:
            print("   ❌ Logout does NOT properly clear the session (BUG FOUND)")
...
def main():
    ...
    try:
        ...
        backend_url = "https://pharmacy-mgmt-portal.preview.emergentagent.com"
    ...
    if len(sys.argv) > 1:
        ...
        elif sys.argv[1] == "--single-session":
            success = tester.test_single_session_per_user()
        else:
            print("Usage: python backend_test.py [--suppliers-only|--users-only|--customers-sales|--categories|--modular|--employee-code|--supplies-employee-code|--state-management|--pwa-offline|--supplier-activation|--single-session]")
            return 1
    else:
